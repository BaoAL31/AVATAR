...
def main() -> int:
    ...
    for suffix, stem in tqdm(stems, desc="mouth-crops", unit="clip"):
        ...
        if not lm_path.is_file():
            ...
            fail_lines.append(f"{suffix},{stem},missing_landmarks_npy")
            continue
        ...
        try:
            ...
            fail_lines.append(f"{suffix},{stem},missing_hub_mp4,{str(e).replace(',', ';')}")
        except ValueError as e:
            failed += 1
            fail_reasons["invalid_data"] += 1
            fail_lines.append(f"{suffix},{stem},invalid_data,{str(e).replace(',', ';')}")
        except Exception as e:
            failed += 1
            fail_reasons["extract_exception"] += 1
            fail_lines.append(f"{suffix},{stem},extract_exception,{type(e).__name__}:{str(e).replace(',', ';')}")
    ...
    if args.fail_log is not None:
        ...
        with fail_log.open("w", encoding="utf-8") as f:
            f.write("suffix,stem,reason,detail\n")
