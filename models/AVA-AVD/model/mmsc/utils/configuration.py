...
from mmsc.utils.general import get_absolute_path, get_mmsc_root
...
def load_yaml(f):
    ...
    try:
        ...
        relative = os.path.abspath(os.path.join(get_mmsc_root(), f))
    ...
    mmsc_root_dir = get_mmsc_root()

    for include in includes:
        original_include_path = include
        include = os.path.join(mmsc_root_dir, include)

        # If path doesn't exist relative to MMSC root, try relative to current file
...
def get_mmsc_cache_dir():
    config = get_global_config()
    cache_dir = config.env.cache_dir
    # If cache_dir path exists do not join to mmsc root
    if not os.path.exists(cache_dir):
        cache_dir = os.path.join(get_mmsc_root(), cache_dir)
    return cache_dir


def get_mmsc_env(key=None):
    config = get_global_config()
    if key:
...
def resolve_cache_dir(env_variable="MMSC_CACHE_DIR", default="mmsc"):
    ...
    if not PathManager.exists(cache_path):
        try:
            ...
            cache_path = os.path.join(get_mmsc_root(), ".mmsc_cache")
            PathManager.mkdirs(cache_path)
...
class Configuration:
    ...
    def import_user_dir(self):
        # Try user_dir options in order of MMSC configuration hierarchy
